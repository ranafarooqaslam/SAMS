...
public class NoOrderDao_Impl implements NoOrderDao {
  ...
  private final EntityDeletionOrUpdateAdapter __deletionAdapterOfNoOrderItem;
  ...
  public NoOrderDao_Impl(RoomDatabase __db) {
    ...
    this.__deletionAdapterOfNoOrderItem = new EntityDeletionOrUpdateAdapter<NoOrderItem>(__db) {
      @Override
      public String createQuery() {
        return "DELETE FROM `NoOrderItem` WHERE `OutletID` = ?";
      }
  ...
  @Override
  public void delete(NoOrderItem item) {
    __db.beginTransaction();
    try {
      __deletionAdapterOfNoOrderItem.handle(item);
      __db.setTransactionSuccessful();
    } finally {
      __db.endTransaction();
    }
