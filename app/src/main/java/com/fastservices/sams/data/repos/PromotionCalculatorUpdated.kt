...
class PromotionCalculatorUpdated(
    val login: UserInfo,
    ...
    var ObjStandardDiscount_Distrbutor = 0.0
    var mGSTPrice = 0.0
    ...
    fun GetPromotion(): ArrayList<dtFreeSKUDetail>? {
        ...
        try {
            ...
            ObjStandardDiscount_Distrbutor = 0.0
            ObjTotalSED = 0.0
            mGSTPrice = 0.0
            ObjGrossSales = 0.0
            ObjTotalGST = 0.0
            ...
            var tempPromotion: dtPromotionOffer?
            var tempSKU: dtFreeSKUDetail?
            ...
            for (PromotionIdx in dtProm.indices) {
                val rowProm = dtProm[PromotionIdx]
                ...
                val pVolume =
                    SamsApplication.getDB().promotionValueDao().getAll(rowProm.promotioN_ID)
                //  SamsApplication.getDB().promotionValueDao().getAll()
                if (pVolume.isNotEmpty()) {
                    for (i in pVolume.indices) {
                        val rowVolume = pVolume[i]
                        if (Customer.promotionClass == rowVolume.customeR_VOLUMECLASS_ID) {
...
//                                dbhelper.getBaskMaser(rowProm.getPROMOTION_ID())
                        for (BMastterIndex in dtBMaster.indices) {
                            ...
                            val rowBMaster = dtBMaster[BMastterIndex]
...
//                                    dbhelper.getBaskDetail(rowBMaster.getBASKET_ID())
                            for (BDetailIndex in dtBDetail.indices) {
                                val rowBDetail = dtBDetail[BDetailIndex]
                                if (rowBDetail.skU_ID > 0)
                                // Single SKU
                                {
                                    if (rowBDetail.skU_ID == tempObj.sKUID) {//If promotion is applies on Single SKU
                                        ...
                                        if (dValueToCompare >= rowBDetail
                                            ...
                                            val dtPOffer =
                                                SamsApplication.getDB().promotionOfferDao()
                                                    .getAll(rowBDetail.baskeT_DETAIL_ID)
                                    ...
                                    if (!Applygroup) {
                                        ...
                                        if (rowBMaster.baskeT_ON == 83) {
                                            for (k in myShoping.indices) {
                                                ...
                                                val groupdetail =
                                                    SamsApplication.getDB().skuGroupDao().getAll(
                                                        tempObj2.sKUID,
                                                        rowBDetail.skU_GROUP_ID
                                                    )
...
//                                                    dbhelper.getPromotionOffer(rowBDetail.baskeT_DETAIL_ID)
                                            if (dtPOffer.isNotEmpty()) {
                                                ...
                                                if (dValueToCompare > rowBDetail
                                                    ...
                                                    val TotalvalueOngroup = rowPOffer
                                                        .offeR_VALUE!! * iMultiply
                                                    tempPromotion.offeR_VALUE =
                                                        TotalvalueOngroup / dValueToCompare
                                                }
                                                else {
                                                    tempPromotion.quantity = rowPOffer
                                                        .quantity
            ...
            Log.d("ZKB1989", Gson().toJson(myPromotions))

            for (p in myPromotions.indices) {
                val myPromotions2 = myPromotions[p]
                if (myPromotions2.claimable == 1) // if Promotion is Claimable Type
                {
                    ...
                    if (myPromotions2.skU_ID > 0) {
                        for (i in myShoping.indices) {
                            ...
                            val tempObj = myShoping[i]
                            if (tempObj.sKUID == myPromotions2.skU_ID) {
                                if (myPromotions2.iS_AND == 1) {
                                    ...
                                    if (myPromotions2.offeR_VALUE > 0) {
                                        tempObj.standardDiscount =
                                            tempObj.standardDiscount + myPromotions2.offeR_VALUE!!
                                        tempObj.claimExtraAmount = tempObj
                                            .claimExtraAmount + myPromotions2
                                            .offeR_VALUE!!
                                        ...
                                        try {
                                            if (myPromotions2.offeR_VALUE_TYPE > 0)
                                            // Trade
                                            // Offer
                                            // Type
                                            {
                                                tempObj.sedAmountByValueTo =
                                                    tempObj.sedAmountByValueTo + myPromotions2.offeR_VALUE!!
                                ...
                                if (myPromotions2.FreeSKUID > 0) { // if Promotion Offer is of Free SKU
                                    val dtsku = SamsApplication.getDB().SKUDao()
                                        .getAll(myPromotions2.FreeSKUID)
//                                            dbhelper.getSKU(myPromotions2.freeSKUID)
                                    if (dtsku.size > 0) {
                                        val rowSku = dtsku[0]
                                        mGSTPrice = rowSku.TRADE_PRICE
                                        tempSKU = dtFreeSKUDetail()
                                        val Row_Id = FreeSKUExist(
                                            dtFree,
                                        ...
                                        //If this SKU not exist already in Free SKU list
                                        {
                                            ...
                                            tempSKU.uniT_PRICE = rowSku.TRADE_PRICE
                                            ...
                                            if (rowSku.GST_ON.contains("T")) {
                                                if (Customer.registered == 0) {
                                                    val threersntgstAmnt =
                                                        rowSku.EXTR_Tax / 100 * rowSku.TRADE_PRICE * myPromotions2.quantity.toDouble()
                                                    tempSKU.gsT_AMOUNT =
                                                        (rowSku.GST_RATE / 100 * rowSku.TRADE_PRICE * myPromotions2.quantity.toDouble()) + threersntgstAmnt
                                                } else {
                                                    tempSKU.gsT_AMOUNT =
                                                        (rowSku.GST_RATE / 100 * rowSku.TRADE_PRICE * myPromotions2.quantity.toDouble())
                                                }
                        ...
                        if (myPromotions2.FreeSKUID > 0) { //If Promotion Offer is on Free SKU
                            val dtsku =
                                SamsApplication.getDB().SKUDao().getAll(myPromotions2.FreeSKUID)
//                                    dbhelper.getSKU(myPromotions2.freeSKUID)
                            if (dtsku.isNotEmpty()) {
                                val rowSku = dtsku[0]
                                mGSTPrice = rowSku.TRADE_PRICE
                                ...
                                // Add New SKU to Free SKU List
                                {
                                    ...
                                    // offer
                                    {
                                        tempSKU.quantity_TO = myPromotions2.quantity
                                    } else
                                    // Additional
                                    {
                                        tempSKU.quantity_AD = myPromotions2.quantity
                                    }
                                    if (rowSku.GST_ON.contains("T")) {
                                        tempSKU.gsT_AMOUNT = (rowSku.GST_RATE / 100
                                                * rowSku.TRADE_PRICE
                                    ...
                                    // Distributor
                                    {
                                        ...
                                        {
                                            if (rowSku.GST_ON.contains("T")) {
                                                tempSKU.extra_tax = tempSKU.amount * rowSku.EXTR_Tax / 100
                                            }
                                        }
                                    }
                                    tempSKU.promotioN_ID = myPromotions2
                                ...
                                {
                                    val rowUpdateSku = dtFree[Row_Id]
                                    ObjFreeGSTAmount -= rowUpdateSku.gsT_AMOUNT
                                    ObjFreeTST -= rowUpdateSku.tsT_AMOUNT
                                    ObjFreeExtra_Tax -= rowUpdateSku.extra_tax
                        ...
                        for (i in myShoping.indices) {
                            val tempObj = myShoping[i]
                            val groupdetail = SamsApplication.getDB().skuGroupDao()
                                .getAll(tempObj.sKUID, myPromotions2.group_ID)
                            ...
                            if (groupdetail.size > 0) {
                                if (myPromotions2.iS_AND == 1) {
                                    ...
                                    if (myPromotions2.discount > 0) {
                                        tempObj.standardDiscountPer = tempObj
                                            .standardDiscountPer + myPromotions2.discount / 100
                                        tempObj.claiM_PER = tempObj
                                            .claiM_PER + myPromotions2.discount / 100
                                        tempObj.sedAmountPerValue = tempObj
                                            .sedAmountPerValue + myPromotions2.discount / 100
                                        // Discount Segregation
                                        try {
                                    ...
                                    if (dtsku.isNotEmpty()) {
                                        ...
                                        // Add
                                        {
                                            ...
                                            // Distributor
                                            {
                                                ...
                                                {
                                                    if (rowSku.GST_ON.contains("T")) {
                                                        tempSKU.extra_tax = 0.0
                                                        if (Customer.registered == 1) {
                                                            tempSKU.gsT_AMOUNT =
                                                                tempSKU.gsT_AMOUNT //added this line by abdul at 23-june-23
                                                        }
                                                        else {
                                                            tempSKU.extra_tax = tempSKU.amount * rowSku.EXTR_Tax / 100
                                                            tempSKU.gsT_AMOUNT =
                                                                tempSKU.gsT_AMOUNT + tempSKU.extra_tax //added this line by abdul at 23-june-23
                                                        }
                                                    }
                                                }
                                            }
                                            ...
                                            val rowUpdateSku = dtFree[Row_Id]
//                                            ObjFreeGSTAmount = ObjFreeGSTAmount - rowUpdateSku.gsT_AMOUNT//commented by abdul
                                            ObjFreeTST -= rowUpdateSku
                                                .tsT_AMOUNT
                                            ObjFreeExtra_Tax -= rowUpdateSku
                                                .extra_tax
...
//                                    dbhelper.getSKU(myPromotions2.freeSKUID)
                            if (dtsku.isNotEmpty()) {
                                ...
                                // Add
                                {
                                    ...
                                    tempSKU.skU_Name = rowSku.SKU_NAME
                                    tempSKU.uniT_PRICE = rowSku.TRADE_PRICE
                                    tempSKU.quantity = myPromotions2.quantity
                                    tempSKU.gsT_RATE = rowSku.GST_RATE
                                    tempSKU.amount = rowSku.TRADE_PRICE * tempSKU.quantity
                                    tempSKU.gsT_AMOUNT = 0.0
                                    tempSKU.tsT_AMOUNT = 0.0
                        ...
                        else if (myPromotions2.offeR_VALUE > 0) { // for distributor discount
                            for (i in myShoping.indices) {
                                val tempObj = myShoping[i]
                                ObjStandardDiscount_Distrbutor += tempObj.quantity * myPromotions2.offeR_VALUE!!
                            }
                        }
...
//                            val groupdetail = dbhelper.getSKUGroupDetail(tempObj.getSKU_ID(), myPromotions2.group_ID)
                            if (groupdetail.isNotEmpty()) {
                                if (myPromotions2.iS_AND == 1) {
                                    ...
                                    if (myPromotions2.offeR_VALUE > 0) {
                                        val Value = tempObj.quantity * myPromotions2.offeR_VALUE!!
                                        tempObj.standardDiscount = tempObj.standardDiscount + Value
                                        // Discount Segregation
                                        try {
                                            if (myPromotions2.offeR_VALUE_TYPE == 0
                                            ) {
            ...
            for (i in myShoping.indices) {
                ...
                if (tempObj.OutletID == Customer.outletID) {
                    ...
                    tempObj.sedAmountPerValueAd = tempObj.sedAmountPerValueAd * tempObj.amount
                    ObjTotalSED_BY_VALUE_AD += tempObj.sedAmountByValueAD
                    ObjTotalSED_PER_VALUE_AD += tempObj.sedAmountPerValueAd
                    if (tempObj.skuItem.GST_ON.contains("T")) {
                        tempObj.gSTAmount = (tempObj.amount - tempObj.standardDiscount) * tempObj.gSTRate / 100
                    }
...
//                            tempObj.gSTAmount =
//                                tempObj.skuItem.DISTRIBUTOR_PRICE * tempObj.gSTRate / 100
...
//                    }
                    ...
                    tempObj.standardDiscountByValueAd = tempObj.standardDiscount
                    ObjStandardDiscount_By_VALUE += tempObj.standardDiscount
                    tempObj.standardDiscountPerValue = tempObj
                        .standardDiscountPer * tempObj.amount
                    tempObj.standardDiscount =
                        tempObj.standardDiscount + tempObj.standardDiscountPer * tempObj.amount
                    tempObj.sedAmountPerValueTo = tempObj
                    ...
                    tempObj.sedAmount = tempObj.claiM_PER * tempObj
                        .amount + tempObj.claimExtraAmount
                    val sed = tempObj.amount * tempObj.gSTRate / 100
                    ObjStandardDiscount += tempObj.standardDiscount
                    ObjTotalSED = (ObjTotalSED + tempObj.claiM_PER * sed + tempObj.claimExtraAmount)
                    ObjTotalSED_BY_VALUE_TO += tempObj.sedAmountByValueTo
                    ObjTotalSED_PER_VALUE_TO += tempObj.sedAmountPerValueTo
                    ObjStandardDiscount_PER_VALUE += tempObj.standardDiscountPerValue
                    ObjTotalSED_BY_VALUE += tempObj.sedAmountByValue
                    ObjTotalSED_PER_VALUE += tempObj.sedAmountPerValue
                    ObjStandardDiscount_By_VALUE_TO += tempObj.standardDiscountByValueTo
                    ObjStandardDiscount_By_VALUE_AD += tempObj.standardDiscountByValueAd
                    ObjStandardDiscount_PER_VALUE_TO += tempObj.standardDiscountPerValueTo
                    ObjStandardDiscount_PER_VALUE_AD += tempObj.standardDiscountPerValueAd

                    ObjTotalTST += tempObj.tSTAmount
...
//                                        dbhelper.getSKU(tempObj.sKUID)
                                if (dtsku.isNotEmpty()) {
                                    val rowSku = dtsku[0]
//                                    tempObj.extraTax =
//                                        (tempObj.amount - (tempObj.standardDiscount - (ObjTotalSED
//                                                + tempObj.claiM_PER * (tempObj.amount * tempObj.gSTRate / 100)
//                                                + tempObj.claimExtraAmount))) * rowSku.EXTR_Tax / 100
                                    tempObj.extraTax = 0.0


//                                    ObjTotalExtra_Tax += (tempObj.amount - totalCalcualtedDistDiscount) * rowSku.EXTR_Tax / 100
                                }
                            }
...
////                                    tempObj.extraTax =
////                                        (tempObj.skuItem.TRADE_PRICE - (tempObj.sedAmount)) * rowSku.EXTR_Tax / 100
////                                    ObjTotalExtra_Tax += (tempObj.skuItem.TRADE_PRICE) * rowSku.EXTR_Tax / 100
//
...
//                                    tempObj.extraTax = (tempObj.amount - (tempObj.standardDiscount - tempObj.sedAmount)) * rowSku.EXTR_Tax / 100
                                    tempObj.extraTax = (tempObj.amount - (tempObj.standardDiscount)) * rowSku.EXTR_Tax / 100
//                                    ObjTotalExtra_Tax += (tempObj.amount - (tempObj.standardDiscount - (ObjTotalSED_BY_VALUE_AD + ObjTotalSED_PER_VALUE_AD))) * rowSku.EXTR_Tax / 100
                                    ObjTotalExtra_Tax += (tempObj.amount - (tempObj.standardDiscount)) * rowSku.EXTR_Tax / 100
                                }
                            }
                        }
                    }
...
//                                        dbhelper.getSKU(tempObj.sKUID)
                                if (dtsku.isNotEmpty()) {
                                    val rowSku = dtsku[0]
//                                    val u_threersntgstAmnt = (tempObj.amount - totalCalcualtedDistDiscount) * rowSku.EXTR_Tax / 100
//                                    tempObj.gSTAmount = ((tempObj.amount - totalCalcualtedDistDiscount) * tempObj.gSTRate / 100) + u_threersntgstAmnt
                                    val u_threersntgstAmnt = (tempObj.amount - tempObj.standardDiscount) * rowSku.EXTR_Tax / 100
                                    tempObj.gSTAmount = (tempObj.amount - tempObj.standardDiscount) * tempObj.gSTRate / 100 + u_threersntgstAmnt
                                    var a = tempObj.gSTAmount
                                }
                            }
                        }
                            ...
                            tempObj.gSTAmount =
                                tempObj.skuItem.TRADE_PRICE * tempObj.gSTRate / 100
                            tempObj.gSTAmount *= tempObj.quantity
                            tempObj.gSTAmount =
                                (tempObj.amount - (tempObj.standardDiscount - (ObjTotalSED_BY_VALUE_AD + ObjTotalSED_PER_VALUE_AD))) * tempObj.gSTRate / 100
//                            tempObj.gSTAmount = (tempObj.amount - (tempObj.standardDiscount - (ObjTotalSED_BY_VALUE_AD + ObjTotalSED_PER_VALUE_AD))) * tempObj.gSTRate / 100
                            tempObj.gSTAmount =
                                (tempObj.amount - tempObj.standardDiscount) * tempObj.gSTRate / 100
//                            tempObj.gSTAmount *= tempObj.quantity
                        }
//                        else {
//                            tempObj.gSTAmount = (tempObj.amount - (tempObj
...
//                        }
                    ...
                    ObjTotalGST += tempObj.gSTAmount
                }
                val objStaVal =
                    ObjStandardDiscount_By_VALUE + ObjStandardDiscount_PER_VALUE + ObjStandardDiscount_By_VALUE_TO + ObjStandardDiscount_By_VALUE_AD
                + ObjStandardDiscount_PER_VALUE_TO + ObjStandardDiscount_PER_VALUE_AD + ObjTotalSED_BY_VALUE + ObjTotalSED_PER_VALUE
            }

            val distDiscount =
                ObjGrossSales - ((ObjStandardDiscount_PER_VALUE - ObjTotalSED_PER_VALUE) - (ObjTotalSED_BY_VALUE_TO + ObjTotalSED_PER_VALUE_TO) - (ObjTotalSED_BY_VALUE_AD + ObjTotalSED_PER_VALUE_AD))
...
//                ObjGrossSales - ObjStandardDiscount + ObjTotalGST + ObjTotalExtra_Tax + ObjTotalTST + ObjFreeTST + ObjFreeGSTAmount
                ObjGrossSales - ObjStandardDiscount + ObjTotalGST + ObjTotalTST + ObjFreeTST + ObjFreeGSTAmount
            )
...
//                    .show()
    ...
    fun saveOrderMaster(
        remarks: String, timeIn: String, timeOut: String,
        latitude: Double, longitude: Double, images: java.util.ArrayList<String>, paymentType: Int
    ): Long {
...
//            ObjStandardDiscount,
            ObjStandardDiscount_Distrbutor,
            summaryUIModel.gst - ObjTotalTST - ObjTotalExtra_Tax,
...
//            ObjStandardDiscount_By_VALUE,// Standard_Discount_By_Value
            ObjStandardDiscount_Distrbutor,// Standard_Discount_By_Value
    ...
    fun saveOrderDetail(
        orderMasterRowId: Long,
        distributionID: String,
        freeSkus: ArrayList<dtFreeSKUDetail>?
    ) {
