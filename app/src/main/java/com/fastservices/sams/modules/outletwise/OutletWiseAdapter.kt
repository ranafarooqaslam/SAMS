...
class OutletVH(val view: View) : androidx.recyclerview.widget.RecyclerView.ViewHolder(view) {
    ...
    val tvOrderTime = view.findViewById<TextView>(R.id.tvOrderTime)

    fun bind(item: OutletWiseSummaryModel) {

        tvCode.text = item.OUTLET_CODE.toString()
        tvName.text = item.OUTLET_NAME
        val timeArray: List<String> = item.ORDER_DATE.split(" ")
        val timeArrayFinal: List<String> = timeArray[1].split(".")
        tvOrderTime.text = timeArrayFinal[0]
        //tvOrderTime.text = item.ORDER_DATE
