    ...
    fun GetPromotion(): ArrayList<dtFreeSKUDetail>? {
        ...
        try {
            ...
            for (PromotionIdx in dtProm.indices) {
                ...
                val pType =
                    SamsApplication.getDB().promotionCustomerDao().getAll(rowProm.promotioN_ID)
...
//                        dbhelper = DBHelper(this@OrderSummery)
                        ...
                        val dtBMaster =
                            SamsApplication.getDB().basketMasterDao().getAll(rowProm.promotioN_ID)
//                                dbhelper.getBaskMaser(rowProm.getPROMOTION_ID())
                        for (BMastterIndex in dtBMaster.indices) {
                            ...
                            val dtBDetail = SamsApplication.getDB().basketDetailDao()
                                .getAll(rowBMaster.baskeT_ID)
                            ...
                            for (BDetailIndex in dtBDetail.indices) {
                                ...
                                // Single SKU
                                {
                                    if (rowBDetail.skU_ID == tempObj
                                            .sKUID
                                    ) {//If promotion is applies on Single SKU
...
//                    ajeeb c bakwas logic hy
                    tempObj.netAmount =
                        tempObj.amount - tempObj.standardDiscount + tempObj.gSTAmount + tempObj.tSTAmount
                    tempObj.gSTPrice =
...
//                                        dbhelper.getSKU(tempObj.sKUID)
                                if (dtsku.isNotEmpty()) {
                                    val rowSku = dtsku[0]
                                    tempObj.extraTax =
                                        (tempObj.amount - (tempObj.standardDiscount - tempObj.sedAmount)) * rowSku.EXTR_Tax / 100
...
//                                        dbhelper.getSKU(tempObj.sKUID)
                                if (dtsku.isNotEmpty()) {
                                    val rowSku = dtsku.get(0)
                                    val u_threersntgstAmnt =
                                        (tempObj.amount - totalCalcualtedDistDiscount) * rowSku.EXTR_Tax / 100


                                    tempObj.gSTAmount =
                                        ((tempObj.amount - totalCalcualtedDistDiscount) * tempObj.gSTRate / 100) + u_threersntgstAmnt
